...
| ORM      | Hibernate / Spring Data JPA          | Parameterised queries (no SQL injection surface); `@SoftDelete` for logical deletion                                                            |
...
**Partial index.** `idx_payments_active_by_time` covers only non-deleted rows (`WHERE deleted = false`), keeping the index small as soft-deleted records accumulate.
...
The idempotency key is checked in a Caffeine cache (24h TTL, 10k entries) before hitting the database. New payments are eagerly written into the cache. The database unique index is the ultimate safeguard: if two concurrent requests race past the cache lookup, the loser gets a `DataIntegrityViolationException`, catches it, retries, and finds the winner's record.
...
`@SoftDelete` keeps deleted payments queryable for audit but means the table grows indefinitely. No archival or hard-delete strategy exists. Acceptable for the current scope; production would need a retention policy.
...
|------------------|-----|----------|--------------------|---------|
| `currencies`     | 24h | 500      | Yes                | Avoids DB lookups for currency reference data |
